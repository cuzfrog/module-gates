...
let tmp: string;
beforeEach(() => {
  tmp = fs.mkdtempSync(path.join("/tmp", "pmg-cli-"));
});
afterEach(() => {
...
describe("module-gates CLI", () => {
  ...
  it("install-devin writes hooks.v1.json with the PreToolUse entry", () => {
    ...
    expect(pre.some((m: { hooks: { command: string }[] }) => m.hooks.some((h) => h.command.includes("@cuzfrog/module-gates")))).toBe(true);
    expect(json.PostToolUse).toBeUndefined();
  });
  ...
  it("end-to-end: install devin, invoke write hook, reject with reason, uninstall", () => {
