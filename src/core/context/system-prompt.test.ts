...
describe("buildSystemPromptHint", () => {
  it("returns augmented prompt when contracts exist", () => {
    ...
    const result = buildSystemPromptHint(index, "You are a helpful assistant.", baseConfig);
  ...
  it("appends module descriptor section to prompt", () => {
    ...
    const result = buildSystemPromptHint(index, "Base prompt.", { ...baseConfig, moduleDescriptorFileName: "CONTEXT.md" });
  ...
  it("includes module interface import rule when gate is enabled", () => {
    ...
    const result = buildSystemPromptHint(index, "Base prompt.", baseConfig);
